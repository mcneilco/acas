...
exports.genericExperimentSearch = (req, res) ->
	if global.specRunnerTestmode 
		...
		authorRoutes.allowedProjectsInternal req.user, (statusCode, allowedUserProjects) ->
			...
			baseurl = config.all.client.service.persistence.fullpath+"experiments/search?q="+encodeURIComponent(req.params.searchTerm)+"&includeDeleted="+includeDeleted+"&projects=#{encodeURIComponent(allowedProjectCodes.join(','))}"
			console.log "baseurl"
			console.log baseurl
