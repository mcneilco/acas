...
checkInstrumentType <- function(assayFileName, instrument, tempFilePath){
  ...
  if (!file.exists(file.path("public/src/modules/PrimaryScreen/src/conf/instruments",instrument)) || 
        !file.exists(file.path("public/src/modules/PrimaryScreen/src/conf/instruments",instrument,"instrumentType.json")) ||
        !file.exists(file.path("public/src/modules/PrimaryScreen/src/conf/instruments",instrument,"detectionLine.json")) ||
        !file.exists(file.path("public/src/modules/PrimaryScreen/src/conf/instruments",instrument,"paramList.json"))) {
    stopUser("Instrument not loaded in to system.")
  } 
  
  instrumentType <- fromJSON(readLines(file.path("public/src/modules/PrimaryScreen/src/conf/instruments",instrument,"instrumentType.json")))$instrumentType
  if(instrumentType != instrument) {
    stopUser("Instrument data loaded incorrectly.")
  }
  
  detectionLine <- fromJSON(readLines(file.path("public/src/modules/PrimaryScreen/src/conf/instruments",instrument,"detectionLine.json")))$detectionLine
