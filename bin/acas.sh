...
# Starts the server.
do_start() {
    LOCKFILE=$ACAS_HOME/bin/acas.LOCKFILE
    ...
    if running; then
        log "acas already running"
    else
        action "starting acas" start_server
        RETVAL=$?
        if [ $RETVAL -eq 0 ]; then
            ...
            if running; then
                ...
                log "acas died on startup" "failure"
                RETVAL=1
            fi
        fi
...
do_stop() {
    ...
    if running; then
        ...
        action "stopping acas" stop_server
        RETVAL=$?
