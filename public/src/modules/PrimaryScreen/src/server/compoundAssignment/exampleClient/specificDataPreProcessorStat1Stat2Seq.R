specificDataPreProcessorStat1Stat2Seq <- function(parameters=parameters, folderToParse=folderToParse, errorEnv=errorEnv, dryRun=dryRun, 
  ...
  #work on making result table output something like instrumentData$plateAssocDT and instrumentData$assayData
  
  resultList <- apply(fileNameTable,1,combineFiles)
  resultTable <- as.data.table(do.call("rbind",resultList))
  assayData <- data.table(assayFileName=resultTable$fileName, assayBarcode=resultTable$barcode,
  ...
  instrumentData <- list(assayData, plateAssociationDT)
