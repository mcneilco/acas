...
saveData <- function(subjectData, treatmentGroupData, analysisGroupData, user, configList, experimentId){
  ...
  # TODO: remove stateID to combine replicates, or use the original treatmentGroupData (but need to maintain a id for treatment groups and analysis groups in the subjects)
  treatmentGroupData <- ddply(.data = treatmentDataStart, .variables = c("stateID", "treatmentGroupID", "resultTypeAndUnit", "stateGroupIndex"), .fun = createRawOnlyTreatmentGroupData)
