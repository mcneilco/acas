...
context("Testing the function getting the Dap data (microBeta)")
...
rdaTest <- function(newResults, acceptedResultsPath, updateResults = FALSE) {
  if(updateResults) {
    ...
    acceptedResults <- testTable
    return(expect_that(newResults,
                       equals(acceptedResults)))
  }
}

test_that("executeDap functionality (microBeta)", {
  require(data.table)
  require(racas)
  require(testthat)
  ...
  setcolorder(testTable, c("assayBarcode","wellReference","rowName","colName","R1..R1.","cmpdBarcode","plateType","corp_name","batch_number","cmpdConc","supplier","sourceType"))
