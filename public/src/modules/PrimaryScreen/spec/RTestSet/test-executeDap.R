...
test_that("getInstrumentSpecificData & getCompoundAssignments functionality", {
  ...
  instrumentSpecData <- getInstrumentSpecificData(filePath=testFilePath, testMode=TRUE, instrument="flipr", tempFilePath=tempFilePath, readOrder=1, readNames="R1", matchNames=FALSE)
  ...
  instrumentSpecData <- getInstrumentSpecificData(filePath=testFilePath, testMode=TRUE, instrument="microBeta", tempFilePath=tempFilePath, readOrder=1, readNames="R1", matchNames=FALSE)
  getCompoundAssignments(plateData=instrumentSpecData$plateAssociationDT, testMode=TRUE, tempFilePath=tempFilePath, assayData=instrumentSpecData$assayData, originalWD=originalWD)
  testFile <- file.path(tempFilePath, "output_well_data.srf")
