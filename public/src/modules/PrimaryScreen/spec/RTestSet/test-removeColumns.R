...
test_that("removeColumns functionality", {
  ...
  newDT <- suppressWarnings(removeColumns(colNamesToCheck=testColNamesToCheck, colNamesToKeep=testColNamesToKeep, inputDataTable=testInputDataTable))
  
  
  expect_that(removeColumns(colNamesToCheck=testColNamesToCheck, colNamesToKeep=testColNamesToKeep, inputDataTable=testInputDataTable), gives_warning("Removed 2 data columns: 'R4 \\{Kappa}','Epsilon'"))
