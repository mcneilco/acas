...
test_that("adjustColumnsToUserInput functionality", {
  ...
  testInputColumnTable <- data.table(userReadOrder=c(1,2,3), userReadName=c("Alpha", "Beta", "Delta"), activityColName=c("Alpha","Beta","None"), newActivityColName=c("R1 {Alpha}", "R2 {Beta}", "R3 {Delta}"))
  ...
  expect_that(colnames(newDT), equals(c("wellReference","R1 {Alpha}", "R2 {Beta}", "R3 {Delta}")))
