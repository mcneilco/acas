...
getPreferredId2 <- function (entityIds, displayName, testMode=FALSE, preferredIdService = NULL) {
  ...
  if (length(entityIds) > 500) {
    return(rbind(getPreferredId2(entityIds[1:500], displayName, preferredIdService, testMode),
                 getPreferredId2(entityIds[501:length(entityIds)], displayName, preferredIdService, testMode)))
  } else {
    requestIds <- list()
    if (testMode) {
